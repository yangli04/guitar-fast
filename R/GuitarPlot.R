...
.RNAPlotStructure <- function(p, componentWidth, headOrtail, pos)
{
  ...
  # Vectorized: single annotate calls for all components at once
  p <- p + annotate("text",
                     x = componentStructure$mid,
                     y = pos$rna_comp_text,
                     label = componentStructure$label)
  p <- p + annotate("rect",
                     xmin = componentStructure$start,
                     xmax = componentStructure$end,
                     ymin = pos$rna_lgd_bl - componentStructure$lgd_height,
                     ymax = pos$rna_lgd_bl + componentStructure$lgd_height,
                     alpha = componentStructure$alpha,
                     colour = "black")
...
{
  densityList <- vector("list", length(sitesPointsRelative))
  listIdx <- 0L
  ...
  for (GroupName in names(sitesPointsRelative)) {
    ...
    tmp <- data.frame(
      ...
      group = rep(GroupName, times = length(fit1$x))
    )
    ...
    densityList[[listIdx]] <- tmp
  }

  return(do.call(rbind, densityList))
}
